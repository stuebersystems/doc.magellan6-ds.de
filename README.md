...
1. Erstelle einen lokalen Ordner für die Dokumentation, z.B. `c:\docs\magellan6-ds`.

2. Starte die Eingabeaufforderung und wechsle in den Ordner `c:\docs\magellan6-ds`.

3. Tippe den Befehl `git clone https://github.com/stuebersystems/doc.magellan6-ds.de.git` ein, um das Repository zu klonen.
...
1. Öffne die URL `https://github.com/stuebersystems/doc.magellan6-ds.de` in deinem Webbrowser

2. Klicke auf die Schaltfläche `Clone or download` und dann auf `Download ZIP`.

3. Entpacke das Zip-Archiv in einen lokalen Ordner Deiner Wahl, z.B. `c:\docs\magellan6-ds`.
...
1. Starte die Eingabeaufforderung und wechsle in den Ordner `c:\docs\magellan6-ds\src`.
...
4. Um Dir das Ergebnis anzeigen zu lassen, wechsle in den Ordner `c:\docs\magellan6-ds\src\_book` und öffne die Datei `index.html` in Deinem Webbrowser.
